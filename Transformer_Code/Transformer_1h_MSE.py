...
def smape_loss(y_pred, target):
    epsilon = 1e-8  
    loss = 2 * (y_pred - target).abs() / (y_pred.abs() + target.abs() + epsilon)
    return loss.mean()  
...
class TimeSeriesForcasting(pl.LightningModule):
    ...
    def training_step(self, batch, batch_idx):
        ...
        return {"loss": loss, "mae": mae, "smape": smape}
    ...
    def test_step(self, batch, batch_idx):
        ...
        return {"test_loss": loss, "test_mae": mae, "test_smape": smape}
...
class Dataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, idx):
        ...
        trg_in = trg[self.features + [f"{x}_lag_1" for x in self.target] ]
...
def train(
    ...
    output_json = {
        ...
        "test_smape": result_val[0]["test_smape"],
...
if __name__ == '__main__':
    ...
    feature_target_names_path = "data/config_1H.json"
    output_json_path = "models/trained_config_1H.json"
